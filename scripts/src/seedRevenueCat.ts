...
const PROJECT_NAME = "Wallpaper App";

const PRODUCT_IDENTIFIER = "wallpaper_purchase";
const PLAY_STORE_PRODUCT_IDENTIFIER = "wallpaper_purchase:one_time";

const PRODUCT_DISPLAY_NAME = "Wallpaper Purchase";
const PRODUCT_USER_FACING_TITLE = "Wallpaper Purchase";
const PRODUCT_DURATION = "P1M";

const APP_STORE_APP_NAME = "Wallpaper App iOS";
const APP_STORE_BUNDLE_ID = "com.wallpaperapp.ios";
const PLAY_STORE_APP_NAME = "Wallpaper App Android";
const PLAY_STORE_PACKAGE_NAME = "com.wallpaperapp.android";

const ENTITLEMENT_IDENTIFIER = "wallpaper_purchase";
const ENTITLEMENT_DISPLAY_NAME = "Wallpaper Purchase";

const OFFERING_IDENTIFIER = "wallpaper_v1";
const OFFERING_DISPLAY_NAME = "Wallpaper Purchase";

const PACKAGE_IDENTIFIER = "wallpaper_pkg";
const PACKAGE_DISPLAY_NAME = "Single Wallpaper";

const PRODUCT_PRICES = [
  { amount_micros: 1000000, currency: "USD" },
];
...
async function seedRevenueCat() {
  ...
  if (listProjectsError) {
    console.error("List projects error:", JSON.stringify(listProjectsError, null, 2));
    throw new Error("Failed to list projects");
  }

  console.log("Found projects:", existingProjects.items?.map((p) => `${p.name} (${p.id})`).join(", ") ?? "none");
  ...
  if (existingProject) {
    ...
    if (createProjectError) {
      console.error("Create project error:", JSON.stringify(createProjectError, null, 2));
      throw new Error("Failed to create project — no existing project found and creation failed. Make sure the API key has Project configuration Read/Write permissions.");
    }
  ...
  if (priceError) {
    ...
    console.log("Successfully added test store prices ($1.00)");
